if (window.location.pathname === '/test') {
  ...
  function openEditPopup() {
    ...
    // 2. 클릭한 a태그의 id(objectId)를 참조해서, DB에서 데이터를 GET한 뒤 폼에 차례로 입력
    // isCombo == true 일 경우(프리미엄, 와퍼, 치킨버거)와 == false 일 경우 두 가지를 구분해서, 데이터를 읽어와야 한다
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response["result"] === "success") {
            ...
            // 메뉴의 가짓수가 백의 자리를 넘지 않으므로 부득이 while문을 사용함
            ...
            if (nameEng.includes(" (A La Carte)")) {
  ...
  function submit() {
    ...
    if (menuType === "프리미엄" || menuType === "와퍼" || menuType === "치킨버거") {
      ...
      nameEng = singleInfo["nameEng"] + " (A La Carte)";
    }
